...
import type { UseRuleGroup } from '../composables/useRuleGroup';

/**
 * Vue node type (equivalent to ReactNode)
 */
 ...
 * @group Props
 */
export interface TranslationWithLabel extends BaseTranslationWithLabel<VueNode> {}
/**
 ...
 * @group Props
 */
export interface Translations extends BaseTranslations<VueNode> {}
/**
...
export interface ShiftActionsProps extends CommonSubComponentProps {
   ...
   */
  labels?: { shiftUp?: VueNode; shiftDown?: VueNode };
  /**
...
export type ControlElementsProp<F extends FullField, O extends string> = Partial<{
   ...
   * @default DragHandle
   */
  dragHandle: Component<DragHandleProps> | null;
  /**
   ...
   */
  ruleGroupBodyElements: Component<RuleGroupProps & UseRuleGroup>;
  /**
   * Rule group header components.
   *
   * @default RuleGroupHeaderComponents
   */
  ruleGroupHeaderElements: Component<RuleGroupProps & UseRuleGroup>;
  /**
...
export interface Schema<F extends FullField, O extends string> {
  ...
  getValueEditorSeparator(field: string, operator: string, meta: { fieldData: F }): VueNode;
...
/**
 * Return type of {@link useRuleGroupDnD} composable.
 */
...
/**
 * Return type of {@link useRuleDnD} composable.
 */
...
export type QueryBuilderContextProvider<ExtraProps extends object = Record<string, any>> =
  Component<QueryBuilderContextProviderProps & ExtraProps>;

/**
 * Props for {@link QueryBuilder}.
 *
 * Notes:
 * - Only one of `modelValue` or `defaultQuery` should be provided. If `modelValue` is present,
 * then `defaultQuery` should be undefined and vice versa.
 * - If rendered initially with a `modelValue` prop, then `modelValue` must be defined in every
 * subsequent render or warnings will be logged (in non-production modes only).
 *
 * @typeParam RG - The type of the query object, inferred from either the `modelValue` or `defaultQuery` prop.
...
> = RG extends RuleGroupType<infer R> | RuleGroupTypeIC<infer R>
  ? QueryBuilderContextProps<F, GetOptionIdentifierType<O>> & {
      ...
      defaultQuery?: RG;
      /**
       * Query object for controlled components (v-model).
       */
      modelValue?: RG;
      /**
       ...
       * `name` and operator `name`. The element can be any valid Vue node,
       * including a bare string (e.g., "and" or "to") or an HTML element.
       * It will be placed in between value editors when multiple
       ...
       * props like `modelValue`, `onUpdateModelValue`, and `enableDragAndDrop` will be ignored.
       */
      getSubQueryBuilderProps?(
        field: GetOptionIdentifierType<F>,
        misc: { fieldData: F }
       ...
       * For Vue, this is typically handled via v-model, but can also be used directly.
       */
       ...
       * {@link RuleGroupTypeIC} for the `modelValue` or `defaultQuery` prop. The query builder
