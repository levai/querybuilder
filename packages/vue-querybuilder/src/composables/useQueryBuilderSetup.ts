...
import type { UseMergedContext } from './useMergedContext';
...
>(
  ...
  const initialQueryProp = ref(props.modelValue ?? props.defaultQuery).value;
  ...
  // #region `combinators`
  const { optionList: combinators } = computed(() =>
    prepareOptionList({
  ...
  // #region `operators`
  const { optionList: operators } = computed(() =>
    prepareOptionList({
  ...
  const getRuleDefaultOperator = (field: FieldName): OperatorName => {
  ...
  const getValueSourcesMain = (
    field: FieldName,
    operator: OperatorName,
    _misc?: { fieldData: F }
  ) => getValueSourcesUtil<F, OperatorName>(fieldMap[field] as F, operator, getValueSources);

  const getMatchModesMain = (field: FieldName, _misc?: { fieldData: F }) =>
    getMatchModesUtil<F>(fieldMap[field] as F, getMatchModes);

  const getSubQueryBuilderPropsMain = (field: FieldName, misc: { fieldData: F }): Record<string, unknown> =>
    // oxlint-disable-next-line typescript/no-explicit-any
  ...
  const getRuleDefaultValue = <RT extends RuleType = R>(r: RT) => {
  ...
  const createRuleGroup = (independentCombinators?: boolean): RG => {
